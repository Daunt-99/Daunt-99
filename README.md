<div align="center">
  <img src="banner.svg" alt="Vikas Mariyappa" width="100%"/>
</div>

<br/>

<div align="center">

<a href="https://www.linkedin.com/in/vikas-mariyappa-048137266/"><img src="https://img.shields.io/badge/LinkedIn-Vikas%20Mariyappa-0A66C2?style=flat&logo=linkedin&logoColor=white"/></a>
[![GitHub](https://img.shields.io/badge/GitHub-Daunt--99-181717?style=flat&logo=github&logoColor=white)](https://github.com/Daunt-99)
[![Email](https://img.shields.io/badge/Email-vdm230002@utdallas.edu-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:vdm230002@utdallas.edu)

</div>

---

## About Me
---

I'm a Computer Science student at UT Dallas specializing in **AI, Machine Learning, and Data Analytics**. I love building things at the intersection of ML and real-world problems and hope to solve problems in the healthcare indsutry as well.

---

## Featured Projects

### Parkinson's Detect
An ML system that analyzes voice biomarkers to detect early-stage Parkinson's disease with **85%+ accuracy**. Presented at AIMD Spring 2025 — placed 2nd overall.

The core insight: subtle changes in vocal frequency, jitter, and shimmer correlate strongly with neurological degradation long before other symptoms appear. Built a full pipeline from raw audio feature extraction through model training and evaluation.

`Python` `Scikit-learn` `NumPy` `Pandas`

---

### TravelBudgetFX
An AI-powered travel planning platform that automates trip budgeting and generates custom itineraries. Built to solve the gap between travel inspiration and the actual logistics of budgeting across currencies and destinations.

Integrates Gemini AI for natural language itinerary generation with a FastAPI backend and a TypeScript frontend that handles real-time FX conversion.

`FastAPI` `TypeScript` `Gemini AI` `REST API`

---

## Tech Stack

**Languages**

![Python](https://img.shields.io/badge/Python-0d1117?style=flat&logo=python&logoColor=3776AB)
![TypeScript](https://img.shields.io/badge/TypeScript-0d1117?style=flat&logo=typescript&logoColor=3178C6)
![JavaScript](https://img.shields.io/badge/JavaScript-0d1117?style=flat&logo=javascript&logoColor=F7DF1E)
![Java](https://img.shields.io/badge/Java-0d1117?style=flat&logo=openjdk&logoColor=ED8B00)
![C++](https://img.shields.io/badge/C++-0d1117?style=flat&logo=c%2B%2B&logoColor=00599C)
![SQL](https://img.shields.io/badge/SQL-0d1117?style=flat&logo=mysql&logoColor=4479A1)
![HTML/CSS](https://img.shields.io/badge/HTML%2FCSS-0d1117?style=flat&logo=html5&logoColor=E34F26)

**Frameworks & Libraries**

![React](https://img.shields.io/badge/React-0d1117?style=flat&logo=react&logoColor=61DAFB)
![FastAPI](https://img.shields.io/badge/FastAPI-0d1117?style=flat&logo=fastapi&logoColor=009688)
![Next.js](https://img.shields.io/badge/Next.js-0d1117?style=flat&logo=nextdotjs&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-0d1117?style=flat&logo=flask&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-0d1117?style=flat&logo=tensorflow&logoColor=FF6F00)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-0d1117?style=flat&logo=scikitlearn&logoColor=F7931E)
![Pandas](https://img.shields.io/badge/Pandas-0d1117?style=flat&logo=pandas&logoColor=150458)
![NumPy](https://img.shields.io/badge/NumPy-0d1117?style=flat&logo=numpy&logoColor=013243)
![OpenCV](https://img.shields.io/badge/OpenCV-0d1117?style=flat&logo=opencv&logoColor=5C3EE8)

**Tools & Platforms**

![Git](https://img.shields.io/badge/Git-0d1117?style=flat&logo=git&logoColor=F05032)
![AWS](https://img.shields.io/badge/AWS-0d1117?style=flat&logo=amazonaws&logoColor=FF9900)
![Firebase](https://img.shields.io/badge/Firebase-0d1117?style=flat&logo=firebase&logoColor=FFCA28)
![MongoDB](https://img.shields.io/badge/MongoDB-0d1117?style=flat&logo=mongodb&logoColor=47A248)
![Supabase](https://img.shields.io/badge/Supabase-0d1117?style=flat&logo=supabase&logoColor=3ECF8E)
![Figma](https://img.shields.io/badge/Figma-0d1117?style=flat&logo=figma&logoColor=F24E1E)

---

## GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Daunt-99&show_icons=true&theme=github_dark&hide_border=true&count_private=true&bg_color=0d1117&title_color=58a6ff&icon_color=3fb950&text_color=8b949e&cache_seconds=1800" height="155"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Daunt-99&layout=compact&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=58a6ff&text_color=8b949e&cache_seconds=1800" height="155"/>
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Daunt-99&theme=github-dark-blue&hide_border=true&background=0d1117&stroke=21262d&ring=58a6ff&fire=3fb950&currStreakLabel=8b949e" height="155"/>
</div>

---

## A Few Things About Me

- Designed flight simulations for an L1 rocket with UTD's Comet Rocketry team
- Interned at AI startups in Austin, TX and Bangalore, India and currently interning at IBM

---

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=58a6ff&height=60&section=footer&animation=fadeIn" width="100%"/>
</div>
